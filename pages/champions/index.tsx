...
import ChampionSelectArea from "../../components/champion-selectArea";
import ChampionSearch from "../../components/champion-search";
import ChampionLane from "../../components/champion-lane";
...
const reducer = (state: Sides, action: Actions) => {
  ...
  switch (type) {
    case actionsKind.IncreaseTeam:
      ...
      const filteredTeamSize = teamSize.filter(position => state[side][position as keyof Team] !== undefined);
      const updatedTeamSize = teamSize.filter(position => !filteredTeamSize.includes(position));
      const teamSizeFiltered = updatedTeamSize.length > 0 ? updatedTeamSize: filteredTeamSize;
...
mutation findChampions($name : String! = "", $lane : String! = "", $order : String! = "asc"){
...
function Home() {
  ...
  const [findChampion, { data , loading, error }] = useMutation(findChampionMutation);
...
console.log(state.BlueBans)
  ...
  const filterAvailableChampions = (champions: Node[], selectedChampions: string[]): Node[] => {
    return champions.filter(champion => !selectedChampions.includes(champion.name) && !bansChampions.includes(champion.name));
  };
  ...
  const chooseChampions = (champion : Node) => {
    ...
    return dispatch({type: actionsKind.IncreaseTeam, payload: {... champion, lane : selectedLane, side : selectedSide! } })
    }

    return dispatch({type: actionsKind.IncreaseTeam, payload: {...champion, side : selectedSide!} })
  }
  }
...
<Layout title="Titulo">
  <div className="flex flex-row">
    ...
    <ChampionSearch onSearch={onSearch}/>
      ...
      isLoading={loading} 
      isChampionsSelected={isChampionAvailable} 
      isChampionsBanned={isChampionBanned}
      />
      <Teams
        data={state.RedSide}
        side={teamSides.RedSide}
      />
    </div>
    </Layout>
  );
